...
void CD3DRender::D3DMSMine(float x, float y, D3DCOLOR color)
{
	...
	D3DLine(x + 4, y + 3, x + 15, y + 14, color);
	D3DLine(x + 3, y + 4, x + 14, y + 15, color);
	D3DLine(x + 3, y + 13, x + 14, y + 2, color);
	D3DLine(x + 4, y + 14, x + 15, y + 3, color);
}
...
void CD3DRender::D3DMSRozha(float x, float y, int state)
{
	...
	case 1:
		...
		break;
	case 2:
		D3DLine(x + 7, y + 8, x + 12, y + 10, 0xFF000000);
		D3DLine(x + 7, y + 11, x + 12, y + 7, 0xFF000000);
		D3DLine(x + 18, y + 11, x + 23, y + 7, 0xFF000000);
		D3DLine(x + 18, y + 8, x + 23, y + 10, 0xFF000000);
		D3DLine(x + 11, y + 23, x + 20, y + 23, 0xFF000000);
		D3DLine(x + 16, y + 24, x + 19, y + 24, 0xFFFF0000);
		break;
	case 3:
		D3DLine(x + 3, y + 8, x + 28, y + 8, 0xFF000000);
...
void CD3DRender::D3DRMR(float cx, float cy, float re, float ra, D3DCOLOR color, D3DCOLOR bcolor, bool border)
{
	...
	if (SUCCEEDED(Begin(D3DPT_TRIANGLEFAN)))
	{
		...
		for (int i = 0; i <= re; ++i)
		{
			float fx = cx + ra * cos(curect*D3DX_PI / 180.f) - ra * sin(curect*D3DX_PI / 180.f);
			float fy = cy + ra * cos(curect*D3DX_PI / 180.f) + ra * sin(curect*D3DX_PI / 180.f);
			D3DVertex2f(fx, fy);
			curect += 360.f / re;
		}
		End();
